        ...
        <ul class="cart-list"
          ...
          <li class="cart-list-con1">
            <input type="checkbox"
                   ...
                   @change="updateCheckById(cartInfo.skuId,$event)">
          </li>
          ...
          <li class="cart-list-con5">
            <a href="javascript:void(0)"
               class="mins"
               @click="handler('reduce',-1,cartInfo)">-</a>
          ...
          <li class="cart-list-con7">
            <a class="sindelet"
               @click="deleteShopCart(cartInfo.skuId)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cartInfo) {
      ...
      try {
        await this.$store.dispatch('addOrUpdateShopCar', { skuId: cartInfo.skuId, skuNum: disNum });
        this.getData();
      } catch (error) {
    ...
    // 修改选中状态
    async updateCheckById (skuId, event) {
      try {
        let isChecked = event.target.checked ? 1 : 0;
        await this.$store.dispatch('updateCheckById', { skuId: skuId, isChecked })
        this.getData();
      } catch (error) {
        alert(error.message);
      }
