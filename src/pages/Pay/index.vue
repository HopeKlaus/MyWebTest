...
export default {
  ...
  methods: {
    async getPayInfo () {
      let results = await this.$API.reqPayInfo(this.orderId);
      if (results.code == 200) {
        this.payInfo = results.data;
      } else {
    ...
    async open () {
      ...
      this.$alert(`<img src="${url}" alt="">`, '微信扫码支付', {
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          let results = await this.$API.reqPayStatus(this.orderId)
          console.log(results);
          if (results.code == 200) {
            // 如果支付成功
            // 清除定时器
