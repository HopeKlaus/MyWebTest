        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li class="with-x"
                ...
                :key="index">{{attrValue.split(':')[1]}}<i @click="removeProps(index)">×</i></li>
          </ul>
        </div>
        <!--selector-->
            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active:isOne}"
                    @click="changeOrder('1')">
                  <a>综合<i v-show="isOne"
                       :class="{'el-icon-caret-top':isAsc,'el-icon-caret-bottom':isDesc}"></i></a>
                </li>
...
export default {
  ...
  methods: {
    ...
    changeOrder (flag) {
      ...
      let newOrder = '';
      // 点击的是否跟上一次相同 如果相同就修改排序
      if (flag == originFlag) {
        newOrder = `${originFlag}:${originSort == 'desc' ? 'asc' : 'desc'}`;
      } else {
        // 不相同就修改flag且把排序修改成默认降序
        newOrder = `${flag}:${'desc'}`;
      };
  ...
  watch: {
    // 监视路由的变化，当携带的参数发生变化时，重新整理参数发起请求，之后把三级分类的ID清空，保证下次发起请求时携带的分类ID参数只有一个
